...
class SPEN:
    ...
    def rank_based_training(self):
        ...
        obj1 = tf.reduce_sum(
            tf.maximum((self.value_h - self.value_l) * self.margin_weight_ph - self.energy_yh + self.energy_yl, 0.0))
        ...
        self.num_update = tf.reduce_sum(
            tf.cast((self.value_h - self.value_l) * self.margin_weight_ph >= (self.energy_yh - self.energy_yl),
                    tf.float32))
        self.train_step = self.optimizer.minimize(self.objective, var_list=self.spen_variables())
        return self
    ...
    def get_first_large_consecutive_diff(self, xinput=None, inf_iter=None, ascent=True):
        ...
        y_a = self.inference(xinput=xinput, train=True, ascent=ascent, inf_iter=n)
        ...
        print("the two energies", np.average(en_a[-2]), np.average(en_a[-1]))
        print("the two values", np.average(f_a[-2]), np.average(f_a[-1]))
    ...
    def get_first_large_consecutive_diff2(self, xinput=None, inf_iter=None, ascent=True):
        self.inf_objective = self.energy_yp
        self.inf_gradient = self.energy_ygradient
    ...
    def soft_predict(self, xinput=None, train=False, inf_iter=None, ascent=True, end2end=False):
        ...
        if end2end:
            ...
            yp_ind_init = np.random.normal(0, 1,
                                           size=(np.shape(xinput)[0], self.config.output_num * self.config.dimension))
    ...
    def train_unsupervised_batch(self, xbatch=None, verbose=0):
        ...
        if np.size(l_h) > 1:
            _, o1, n1, v1, v2, e1, e2 = self.sess.run(
                [self.train_step, self.objective, self.num_update, self.vh_sum, self.vl_sum, self.eh_sum, self.el_sum],

...
class EnergyModel:
    ...
    def get_energy_rnn_mlp_emb(self, xinput, yinput, embedding=None, reuse=False):
        ...
        with tf.variable_scope(self.config.spen_variable_scope):
            with tf.variable_scope(self.config.fx_variable_scope) as scope:
                ...
                print(yinput.get_shape()) # ?, 630
                #print_out0 = tf.Print(logits2, logits2.get_shape())
                #print_out1 = tf.Print(yinput, [yinput])
    ...
    def get_feature_net_custom(self, xinput, output_num, reuse=False):
        net = xinput
        with tf.variable_scope("bi-lstm1") as scope:
            if reuse:
                scope.reuse_variables()
            cell_fw = tf.contrib.rnn.LSTMCell(self.config.lstm_hidden_size, reuse=reuse)
            cell_bw = tf.contrib.rnn.LSTMCell(self.config.lstm_hidden_size, reuse=reuse)
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                cell_fw, cell_bw, net, dtype=tf.float32, parallel_iterations=10)
            output = tf.concat([output_fw, output_bw], axis=-1)

        with tf.variable_scope("bi-lstm2") as scope:
            if reuse:
                scope.reuse_variables()
            cell_fw = tf.contrib.rnn.LSTMCell(2 * self.config.lstm_hidden_size, reuse=reuse)
            cell_bw = tf.contrib.rnn.LSTMCell(2 * self.config.lstm_hidden_size, reuse=reuse)
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                cell_fw, cell_bw, output, dtype=tf.float32, parallel_iterations=10)
            output = tf.concat([output_fw, output_bw], axis=-1)

        output = tf.reshape(output, [-1, self.config.sequence_length * 4 * self.config.lstm_hidden_size])

        with tf.variable_scope("proj") as scope:
            if reuse:
                scope.reuse_variables()
            W = tf.get_variable("PW", dtype=tf.float32,
                                shape=[self.config.sequence_length * 4 * self.config.lstm_hidden_size, output_num])

            b = tf.get_variable("Pb", shape=[output_num],
                                dtype=tf.float32, initializer=tf.zeros_initializer())
